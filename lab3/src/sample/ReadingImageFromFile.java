...
// який приймає на вхід назву файлу з зображенням у форматі BMP
    public static void loadBitmapImage(String filename) throws IOException {
        ...
        BufferedOutputStream writer = new BufferedOutputStream(new FileOutputStream("files/primer_bmp.txt")); // потік для запису зчитаних зображень про пікселі у ASCII кодах
        while ((line = reader.read()) != -1) {
            writer.write(line);
        }
        reader.close();
        writer.close();
        BufferedInputStream reader1 = new BufferedInputStream(new FileInputStream("files/primer_bmp.txt")); // потік
        ...
        HeaderBitmapImage hbi = reading.Reading(reader1); // у об'єкт типу HeaderBitmapImage записуємо результат роботи методу читання заголовку файлу
        pr = reading.pr;
